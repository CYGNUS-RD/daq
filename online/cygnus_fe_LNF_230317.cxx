...
INT max_event_size =   50000000;

/* maximum event size for fragmented events (EQ_FRAGMENTED) */
INT max_event_size_frag = 5 * 1024 * 1024;

/* buffer size to hold events */
INT event_buffer_size =  100000000;
...
INT frontend_init()
{
  ...
  int status = mvme_open(&gVme,1);		//previously '1'. After Ubuntu's update, only works with '0'
  ...
  */
  ...
  if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_TRIGGERSOURCE" << endl;
  ...
  if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_OUTPUTTRIGGER_POLARITY" << endl;  
  ...
  if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_OUTPUTTRIGGER_KIND + step" << endl;
    
  err = dcamprop_setvalue( gCam, DCAM_IDPROP_OUTPUTTRIGGER_SOURCE + 256, DCAMPROP_OUTPUTTRIGGER_SOURCE__TRIGGER);
  if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_OUTPUTTRIGGER_SOURCE + step" << endl;
  ...
  //if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_OUTPUTTRIGGER_PERIOD + step" << endl;  
  ...
  if(failed(err) && DEBUG) myfile << "ERROR IN DCAM_IDPROP_OUTPUTTRIGGER_POLARITY + step" << endl;
...
{
  ofstream myfile;
  if(DEBUG) myfile.open("debug.txt", ios_base::app);
  if(DEBUG) myfile << "polling..."<<endl;
...
INT read_event(char *pevent, INT off)
{
...
#ifdef HAVE_V1190
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" before read_tdc"<<std::endl<<std::flush;
  read_tdc(pevent);
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" after  read_tdc"<<std::endl<<std::flush;
#endif
  ...
  //if(DEBUG) myfile<<"DEBUG duration up to before read_dgtz: "<< duration.count()/1000. <<" ms"<< endl;
  
  
  if(!freerun) {
      //if(DEBUG && befaft) myfile<<"DEBUG "<<" before read_dgtz"<<std::endl<<std::flush;
      read_dgtz(pevent);
      //if(DEBUG && befaft) myfile<<"DEBUG "<<" after read_dgtz"<<std::endl<<std::flush;
  }
  
  //stop = std::chrono::high_resolution_clock::now();
  //duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  //if(DEBUG) myfile<<"DEBUG duration up to read_dgtz: "<< duration.count()/1000. <<" ms"<< endl;
#endif
  ...
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" before read_camera"<<std::endl<<std::flush;
  
  read_camera(pevent);

    
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" after read_camera"<<std::endl<<std::flush;
  //stop = std::chrono::high_resolution_clock::now();
  //duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  //if(DEBUG) myfile<<"DEBUG duration up to read_camera: "<< duration.count()/1000. <<" ms"<< endl;
  ...
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" before cleardevice"<<std::endl<<std::flush;
    
  //if(DEBUG && befaft) myfile<<"DEBUG "<<" after cleardevice"<<std::endl<<std::flush;

  
  //stop = std::chrono::high_resolution_clock::now();
  //duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  //if(DEBUG) myfile<<"DEBUG duration of read_event: "<< duration.count()/1000. <<" ms"<< endl;
...
INT init_vme_modules(){
  ...
  std::cout<<"DEBUG inside init_vme_modules:"<<std::endl;
  ...
  std::cout<<"DEBUG after first two CAEN_WriteRegister"<<std::endl;
...
e channel 0 and 1*/
	...
	CAEN_DGTZ_SetGroupEnableMask(gDGTZ[i],0xF); 
      ...
      ret |= CAEN_DGTZ_SetMaxNumEventsBLT(gDGTZ[i],256);                                /* Set the max number of events to transfer in a sigle readout */ //128 before fix
  ...
  for(int i=0;i<nboard;i++){
	...
	SAMPLING[i] = 4000;
     ...
     //for_V1720E
      ...
      {
	if(ndgtz[i] > (1.024e6/maxtriggersize) ) ndgtz[i] = (int)(1.024e6/maxtriggersize);
	ret |= CAEN_DGTZ_SetRecordLength(gDGTZ[i],ndgtz[i]);                                /* Set the lenght of each waveform (in samples) */
	SAMPLING[i] = 250;
    ...
    //Disable drs4 corrections
   ret = CAEN_DGTZ_DisableDRS4Correction(gDGTZ[i]);
   if( ret != CAEN_DGTZ_Success) {
...
//         //std::cout<<"DEBUG "<<MAX_X742_GROUP_SIZE<<std::endl<<std::flush; //DEBUG
//         ret = CAEN_DGTZ_GetCorrectionTables(gDGTZ[i], DRS4Frequency, (void*)CTable);
//         if(ret != CAEN_DGTZ_Success) throw std::runtime_error("DEBUG ctables.\n");
//         else {
//             SaveCorrectionTables("./ctables/ctables_-0.5offset_before_DCO", (uint32_t)(pow(2,NCHDGTZ[i]))-1, CTable);
//         }
...
int read_dgtz(char* pevent){
  ...
  for(int i=0;i<nboard;i++){
    ...
    ret = CAEN_DGTZ_ReadData(gDGTZ[i],CAEN_DGTZ_SLAVE_TERMINATED_READOUT_MBLT,buffer_dgtz[i],&bsize);
    //if(DEBUG) myfile<<"DEBUG "<<BoardName[i]<<" "<<ret<<std::endl<<std::flush;
    //if(DEBUG) myfile<<"DEBUG numevents..."<<std::endl<<std::flush;
    ret = CAEN_DGTZ_GetNumEvents(gDGTZ[i],buffer_dgtz[i],bsize,&NumEvents);
    //if(DEBUG) myfile<<"DEBUG "<<BoardName[i]<<" "<<ret<<"  "<<NumEvents<<std::endl<<std::flush;
    NumEvents = std::min(NumEvents, events_max);
       
    
    //NumEvents = std::min(events_max, NumEvents);
    ...
    if(strcmp(BoardName[i],"V1761")==0 || strcmp(BoardName[i],"V1720E")==0){
      ...
      for(int iev=0;iev<NumEvents;iev++){
            //if(DEBUG) myfile<<"DEBUG V1720 "<<iev<<std::endl<<std::flush;
            
    	    ret = CAEN_DGTZ_AllocateEvent(gDGTZ[i], (void**)&Evt);
	    
	    //if(DEBUG) myfile<<"DEBUG V1720 ret "<<ret<<std::endl<<std::flush;
	    ...
	    //tmp_trgttag[event_i] = eventInfo.TriggerTimeTag;
    ...
    //#ifdef HAVE_V1742
	  ...
	  //tmp_trgttag[event_i] = Evt->DataGroup[0].TriggerTimeTag;
          ...
          //event_i++;
          //if(DEBUG) myfile<<"DEBUG "<<ret <<" evt "<<iev<<" info: EVTCNT = "
          //                <<eventInfo.EventCounter<<"  TRGTTAG = "<<Evt->DataGroup[0].TriggerTimeTag<<std::endl;
    ...
    //
    // std::cout<<"\n\n\n\n\n\n\n\n"; 
    //std::cout<<NumEvents<<"  "<<event_i<<std::endl;

    //for(int k = 0; k<event_i; k++) {
    //  std::cout<<tmp_trgttag[k]<<std::endl;
    //}
    
    TRGTTAG[i] = tmp_trgttag;
    EVTSNUM[i] = NumEvents; //event_i
    //if(DEBUG) myfile<<"DEBUG board "<<i<< " nevs = "<<EVTSNUM[i]<<endl;
  ...
  *hdata++ = header_data;
  ...
  for(int i=0;i<nboard;i++){
    ...
    //std::cout<<"=============="<<header_data<<std::endl<<std::flush;
    *hdata++ = header_data;
    ...
    *hdata++ = header_data;
    ...
    header_data = EVTSNUM[i]; //NumEvents; // THIS DOES NOT WORK WITH TWO DGTZ!!!!! CHANGE ASAP
    *hdata++ = header_data;
    ...
    *hdata++ = header_data;
    ...
    for(int j=0;j<NCHDGTZ[i];j++){
      ...
      *hdata++ = header_data;
	    ...
	    for(unsigned int j=0; j<EVTSNUM[i]; j++) {
	      header_data = (uint32_t)TRGTTAG[i][j];
	      *hdata++ = header_data;
    ...
    if(strcmp(BoardName[i],"V1742")==0) {
        for(unsigned int j=0; j<EVTSNUM[i]; j++) {
            header_data = (uint32_t)StartIndexCell[j];
            *hdata++ = header_data;
            if(DEBUG) myfile << "DEBUG start index cell "<<j<<"  "<<StartIndexCell[j]<<std::endl;
        }
    }
