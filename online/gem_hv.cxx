...
static void free_mem(GEM_HV_INFO * hv_info)
{
   free(hv_info->names);
   free(hv_info->demand);
...
INT gem_hv_start(EQUIPMENT * pequipment)
{
   INT i;
...
INT gem_hv_stop(EQUIPMENT * pequipment)
{
   INT i;
...
INT gem_hv_ramp(GEM_HV_INFO * hv_info)
{
...
void gem_hv_set_trip_time(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_set_current_limit(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_set_rampup(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_set_rampdown(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_set_voltage_limit(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_update_label(INT hDB, INT hKey, void *info)
{
...
void gem_hv_set_chState(INT hDB, INT hKey, void *info)
{
   INT i;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
void gem_hv_validate_odb_array(HNDLE hDB, GEM_HV_INFO *hv_info, const char *path, double default_value, int cmd, 
...
void gem_hv_validate_odb_int_read(HNDLE hDB, GEM_HV_INFO *hv_info, const char *path, int default_value, int *target)
{
  int i;
  HNDLE hKey;
  ...
  db_find_key(hDB, hv_info->hKeyRoot, path, &hKey);
  assert(hKey);
  db_open_record(hDB, hKey, target, sizeof(INT), MODE_READ,NULL, NULL);
}

/*------------------------------------------------------------------*/

void gem_hv_validate_odb_array_float_read(HNDLE hDB, GEM_HV_INFO *hv_info, const char *path, float default_value, float *array)
{
  int i;
  HNDLE hKey;
  ...
  db_merge_data(hDB, hv_info->hKeyRoot, path, array, sizeof(float) * hv_info->num_channels, hv_info->num_channels, TID_FLOAT);
  db_find_key(hDB, hv_info->hKeyRoot, path, &hKey);
  assert(hKey);
  db_open_record(hDB, hKey, array, sizeof(float) * hv_info->num_channels, MODE_READ, NULL, NULL);
}

/*------------------------------------------------------------------*/

void gem_hv_validate_odb_array_int_read(HNDLE hDB, GEM_HV_INFO *hv_info, char *path, int default_value, int *array)
{
  int i;
  HNDLE hKey;
  ...
  for (i = 0; i < hv_info->num_channels; i++)
    array[i] = default_value;
  db_merge_data(hDB, hv_info->hKeyRoot, path, array, sizeof(int) * hv_info->num_channels, hv_info->num_channels, TID_INT);
  db_find_key(hDB, hv_info->hKeyRoot, path, &hKey);
  assert(hKey);
  db_open_record(hDB, hKey, array, sizeof(int) * hv_info->num_channels, MODE_READ, NULL, NULL);
}

/*------------------------------------------------------------------*/

INT gem_hv_init(EQUIPMENT * pequipment)
{
   ...
   pequipment->cd_info = calloc(1, sizeof(GEM_HV_INFO));
   hv_info = (GEM_HV_INFO *) pequipment->cd_info;
   ...
   hv_info->current_hotspot = (float *) calloc(hv_info->num_channels, sizeof(float));
   ...
   hv_info->demand_recovery = (float *) calloc(hv_info->num_channels, sizeof(float));
   ...
   /* Update threshold */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Update Threshold Measured", 0.5, CMD_GET_THRESHOLD, 
                      hv_info->update_threshold, NULL, NULL);

   /* Update threshold current */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Update Threshold Current", 1, CMD_GET_THRESHOLD_CURRENT, 
                      hv_info->update_threshold_current, NULL, NULL);

   /* Zero threshold */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Zero Threshold", 20, CMD_GET_THRESHOLD_ZERO, 
                      hv_info->zero_threshold, NULL, NULL);

   /* Voltage limit */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Voltage Limit", 3000, CMD_GET_VOLTAGE_LIMIT, 
                      hv_info->voltage_limit, gem_hv_set_voltage_limit, pequipment);

   /* Current limit */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Current Limit", 3000, CMD_GET_CURRENT_LIMIT, 
                      hv_info->current_limit, gem_hv_set_current_limit, pequipment);

   /* Current hot spot */
   gem_hv_validate_odb_array_float_read(hDB, hv_info, "Settings/Current Hot Spot", 10, hv_info->current_hotspot);

   /* Trip Time */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Trip Time", 10, CMD_GET_TRIP_TIME, 
                      hv_info->trip_time, gem_hv_set_trip_time, pequipment);

   /* Ramp up */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Ramp Up Speed", 0, CMD_GET_RAMPUP, 
                      hv_info->rampup_speed, gem_hv_set_rampup, pequipment);

   /* Ramp down */
   gem_hv_validate_odb_array(hDB, hv_info, "Settings/Ramp Down Speed", 0, CMD_GET_RAMPDOWN, 
   ...
   if (hv_info->driver[0]->flags & DF_REPORT_CRATEMAP){
      ...
      gem_hv_validate_odb_int(hDB, hv_info, str, 'n', CMD_GET_CRATEMAP,
   ...
   db_open_record(hDB, hv_info->hKeyDemand, hv_info->demand,
                  hv_info->num_channels * sizeof(float), MODE_READ, gem_hv_demand,
                  pequipment);
...
INT gem_hv_exit(EQUIPMENT * pequipment)
{
   INT i;

   free_mem((GEM_HV_INFO *) pequipment->cd_info);
...
INT gem_hv_idle(EQUIPMENT * pequipment)
{
   ...
   if (pequipment->info.event_limit == 1) {
      ...
      for (act = 0 ; act < hv_info->num_channels ; act++)
         status = gem_hv_read(pequipment, act);
   } else {
   ...
   for (i = 0 ; i < hv_info->num_channels ; i++){
     ...
     if(hv_info->current[i] > (hv_info->current_hotspot[i])){
       ...
       //trip if current is still large after 10 seconds of recovery
   ...
   if(trip_found==1) {
     ...
     for (i = 0 ; i < hv_info->num_channels ; i++)
       if(hv_info->chStatus[i]!=1){
	 /////TO DO -- ONLY IF NAME MATCHED SOME PATTERN
...
INT cd_gem_hv_read(char *pevent, int offset)
{
   float *pdata;
   GEM_HV_INFO *hv_info;
   EQUIPMENT *pequipment;
...
INT cd_gem_hv(INT cmd, PEQUIPMENT pequipment)
{
   ...
   switch (cmd) {
   case CMD_INIT:
      status = gem_hv_init(pequipment);
      break;

   case CMD_EXIT:
      status = gem_hv_exit(pequipment);
      break;

   case CMD_START:
      status = gem_hv_start(pequipment);
      break;

   case CMD_STOP:
      status = gem_hv_stop(pequipment);
      break;

   case CMD_IDLE:
      status = gem_hv_idle(pequipment);
      break;
