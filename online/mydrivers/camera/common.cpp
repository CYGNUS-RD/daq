...
{
	printf( "FAILED: (DCAMERR)0x%08X @ %s", err, apiname );
...
void dcamcon_show_dcamdev_info( HDCAM hdcam )
{
	...
	{
		dcamcon_show_dcamerr( err, "dcamdev_getstring(DCAM_IDSTR_MODEL)\n" );
	}
	else
	...
	{
		dcamcon_show_dcamerr( err, "dcamdev_getstring(DCAM_IDSTR_BUS)\n" );
	}
	else
	{
		printf( "%s (%s) on %s\n", model, cameraid, bus );
...
void dcamcon_show_dcamdev_info_detail( HDCAM hdcam )
{
	...
	if( ! my_dcamdev_string( err, hdcam, DCAM_IDSTR_VENDOR, buf, sizeof(buf) ) )
		dcamcon_show_dcamerr( err, "dcamdev_getstring(DCAM_IDSTR_VENDOR)\n" );
	else
