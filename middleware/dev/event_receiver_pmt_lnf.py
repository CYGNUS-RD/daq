...
DEFAULT_VMIN_VALUE  = '95'        #Fusion: 95    Quest: 195
DEFAULT_VMAX_VALUE  = '120'       #Fusion: 120    Quest: 220
...
def plot_waveform1(waveform_f, lenw_f, pmt_f, event_number, event_time):
    import numpy as np
    
    t = np.linspace(0,lenw_f, lenw_f)
    #for ipmt in range(pmt_f):
    #    plt.subplot(pmt_f, 2, ipmt*2+2)
    #    plt.plot(t, waveform_f[ipmt])
    plt.subplot(pmt_f, 2, 2)
    plt.plot(t, waveform_f[1])
    plt.subplot(pmt_f, 2, 4)
    plt.plot(t, waveform_f[3])
...
#David please correct this speaking to mazzitelli
def daq_dgz_full2array_local(bank, header, verbose=False, corrected=True, ch_offset=[], tag='LNGS'):
    waveform_f = []
    data_offset = 0
    ...
    if not corrected:              ## Correcting the wavefoms (only the ones with offset at -0.3 of first 8 channels)
        waveform_f = cy.correct_waveforms(waveform_f, SIC[0], number_channels, to_correct=to_correct, tag=tag)
        ...
        try:
            ...
            for bank_name, bank in event.banks.items():
                ...
                if bank_name=='DGH0': 
                    ...
                    #1 digitizer
                    #waveform_header = cy.daq_dgz2header(bank, verbose=False)
                    waveform_header = cy.daq_dgz_full2header(bank, verbose=False)
                    if verbose: print (waveform_header)
                    waveform_fast = daq_dgz_full2array_local(event.banks['DIG0'], waveform_header, verbose=False,corrected=corrected,ch_offset=channels_offset)

                    lenw_fast = waveform_header[2][0]
                    plot_waveform1(waveform_fast, lenw_fast, int(pmt_f), event_number, event_time)
